...
int main()
{
	...
	for(int lambda_index = 0; lambda_index<9;lambda_index++)
	{
		...
		{
			output.open(PATH+"resultRandom\\"+"result_"+ConvertToString( num[pos])+"_"+ConvertToString(lambda_index)+SUFFIX,ios::out|ios::binary|ios::trunc);
			for(unsigned int times=0;times!=NUM_OF_TASK;++times)
			{
				...
				for(map<string,skill*>::iterator it_task = TASK.begin();it_task!=TASK.end();++it_task)
				{
					int randomIndx = rand()%it_task->second->holders.size();
					bestTeam.insert(make_pair(it_task->first,it_task->second->holders[randomIndx]));
				}
