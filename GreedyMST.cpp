...
int main()
{
	...
	{
	output.open(PATH+"resultGM\\"+"result_"+ConvertToString( num[pos])+SUFFIX,ios::out|ios::binary|ios::trunc);
		...
		while(!RET&&AUTHORSET.size()>0)
		{
			...
			else
			{
				...
				for(map<string,author_skill*>::iterator it_au_s = AUTHORSET.begin();it_au_s!=AUTHORSET.end();++it_au_s)
				{
					...
					for(vector<string>::iterator it_NE = GREEDYDIA.begin();it_NE!=GREEDYDIA.end();it_NE++)
					{
						for(vector<string>::iterator it_NE_NE = it_NE+1;it_NE_NE!=GREEDYDIA.end();it_NE_NE++)
						{
					...
					*/
					...
					if(skillToBeCovered.size()/cost_MST>benefit)
					{
						
						benefit = skillToBeCovered.size()/cost_MST;
						auth = it_au_s->first;
						i_star = des;
						cost = cost_MST;
						SKILL.clear();
